export default {
    // Menu
    "menu.dashboard": "Tổng quan",
    "menu.user": "Người dùng",
    "menu.siteSettings": "Cài đặt chung",
    "menu.tag": "Thẻ",
    "menu.category": "Danh mục",
    "menu.post": "Bài viết",
    "menu.comment": "Bình luận",
    "menu.setting": "Cài đặt",

    // Site Settings Page
    "siteSettings.title": "Cài đặt chung",
    "siteSettings.description": "Cấu hình nội dung footer hiển thị trên website blog.",
    "siteSettings.socialLinks.title": "Liên kết mạng xã hội",
    "siteSettings.socialLinks.add": "Thêm liên kết",
    "siteSettings.socialLinks.success": "Cập nhật liên kết thành công!",
    "siteSettings.socialLinks.error": "Cập nhật liên kết thất bại",
    "siteSettings.socialLinks.logoUrl": "URL Logo",
    "siteSettings.socialLinks.logoUrl.required": "Vui lòng nhập URL logo",
    "siteSettings.socialLinks.logoUrl.invalid": "URL không hợp lệ",
    "siteSettings.socialLinks.name": "Tên hiển thị",
    "siteSettings.socialLinks.name.required": "Vui lòng nhập tên",
    "siteSettings.socialLinks.url": "Đường dẫn",
    "siteSettings.socialLinks.url.required": "Vui lòng nhập URL",
    "siteSettings.socialLinks.url.placeholder": "https://...",
    "siteSettings.socialLinks.url.invalid": "URL không hợp lệ",
    "siteSettings.socialLinks.name.placeholder": "Nhãn (ví dụ: Facebook)",
    "siteSettings.socialLinks.logo.placeholder": "URL Logo",
    "siteSettings.contactEmail.title": "Email liên hệ",
    "siteSettings.contactEmail.label": "Địa chỉ Email",
    "siteSettings.contactEmail.required": "Vui lòng nhập email",
    "siteSettings.contactEmail.invalid": "Email không hợp lệ",
    "siteSettings.contactEmail.success": "Cập nhật email thành công!",
    "siteSettings.contactEmail.error": "Cập nhật email thất bại",
    "siteSettings.contactEmail.placeholder": "contact@yourdomain.com",
    "siteSettings.contactEmail.help":
        "Email này sẽ được hiển thị ở chân trang website để khách truy cập có thể liên hệ với bạn.",
    "siteSettings.switch.on": "Bật",
    "siteSettings.switch.off": "Tắt",
    "siteSettings.tab.brand": "Thương hiệu",
    "siteSettings.tab.social": "Mạng xã hội",
    "siteSettings.tab.contact": "Liên hệ",
    "siteSettings.brand.title": "Cài đặt thương hiệu",
    "siteSettings.brand.logo": "Logo",
    "siteSettings.brand.logo.placeholder": "URL hình ảnh logo",
    "siteSettings.brand.logo.description": "Logo và thương hiệu trang web",
    "siteSettings.brand.logo.required": "Vui lòng nhập URL logo",
    "siteSettings.brand.logo.invalid": "URL không hợp lệ",
    "siteSettings.brand.siteTitle": "Tiêu đề website",
    "siteSettings.brand.siteTitle.placeholder": "Nhập tiêu đề website",
    "siteSettings.brand.siteTitle.default": "Tiêu đề trang",
    "siteSettings.brand.siteTitle.required": "Vui lòng nhập tiêu đề",
    "siteSettings.brand.shortIntroVi": "Giới thiệu ngắn (VI)",
    "siteSettings.brand.shortIntroVi.placeholder": "Mô tả ngắn tiếng Việt",
    "siteSettings.brand.shortIntroEn": "Giới thiệu ngắn (EN)",
    "siteSettings.brand.shortIntroEn.placeholder": "Mô tả ngắn tiếng Anh",
    "siteSettings.brand.fullIntroVi": "Giới thiệu đầy đủ (VI)",
    "siteSettings.brand.fullIntroVi.placeholder": "Nội dung chi tiết tiếng Việt (Markdown)",
    "siteSettings.brand.fullIntroEn": "Giới thiệu đầy đủ (EN)",
    "siteSettings.brand.fullIntroEn.placeholder": "Nội dung chi tiết tiếng Anh (Markdown)",
    "siteSettings.brand.success": "Cập nhật thông tin thương hiệu thành công!",
    "siteSettings.brand.error": "Cập nhật thông tin thương hiệu thất bại",

    // App
    "app.title": "Duck Blog Admin",

    // User Menu
    "user.profile": "Hồ sơ",
    "user.logout": "Đăng xuất",

    // Theme
    "theme.light": "Chế độ sáng",
    "theme.dark": "Chế độ tối",
    "theme.system": "Theo hệ thống",

    // Pages
    "page.dashboard.title": "Tổng quan",
    "page.user.title": "Quản lý người dùng",
    "page.tag.title": "Quản lý thẻ",
    "page.category.title": "Quản lý danh mục",
    "page.post.title": "Quản lý bài viết",
    "page.comment.title": "Quản lý bình luận",
    "page.notFound.title": "404 - Không tìm thấy trang",
    "page.notFound.subtitle": "Xin lỗi, trang bạn đang tìm kiếm không tồn tại.",
    "page.notFound.backHome": "Về trang chủ",
    "page.forbidden.subtitle":
        "Xin lỗi, bạn không có quyền truy cập trang này. Chỉ Admin mới có thể truy cập.",
    "page.forbidden.logout": "Đăng xuất",

    // Common
    "common.comingSoon": "Tính năng đang phát triển",
    "common.filter.status": "Lọc theo trạng thái",
    "common.status": "Trạng thái",
    "common.active": "Hoạt động",
    "common.inactive": "Vô hiệu",
    "common.search.placeholder": "Tìm kiếm...",
    "common.yes": "Có",
    "common.no": "Không",

    // Pagination
    "pagination.showTotal": "{range0}-{range1} / {total}",

    // Actions
    "action.title": "Thao tác",
    "action.create.button": "Thêm mới",
    "action.create.title": "Thêm {label}",
    "action.create.success": "Thêm {label} thành công",
    "action.create.error": "Thêm {label} thất bại",
    "action.update.button": "Cập nhật",
    "action.update.title": "Cập nhật {label}",
    "action.update.success": "Cập nhật {label} thành công",
    "action.update.error": "Cập nhật {label} thất bại",
    "action.view.button": "Xem chi tiết",
    "action.save.button": "Lưu",
    "action.cancel.button": "Huỷ bỏ",
    "action.delete.button": "Xoá",
    "action.delete.title": "Xoá {label}",
    "action.delete.alert": "Bạn có chắc muốn xóa {label}? Hành động này không thể hoàn tác.",
    "action.delete.success": "Xoá {label} thành công",
    "action.delete.error": "Xoá {label} thất bại",
    "action.back.button": "Quay lại",
    "action.loading": "Đang tải...",
    "action.upload.button": "Tải lên",

    // Validations
    "validation.required": "Vui lòng nhập {field}.",
    "validation.emailFormat": "Vui lòng nhập đúng định dạng Email!",
    "validation.input": "Vui lòng nhập thông tin!",

    // Upload
    "upload.success": "Tải lên thành công",
    "upload.error": "Tải lên thất bại",
    "upload.uploading": "Đang tải lên...",
    "upload.invalidType": "Loại file không hợp lệ",
    "upload.maxSize": "Kích thước file tối đa là {size}",

    // User Table
    "user.table.title": "Danh sách người dùng",
    "user.table.avatar": "Ảnh",
    "user.table.name": "Tên",
    "user.table.email": "Email",
    "user.table.role": "Vai trò",
    "user.table.verified": "Xác thực",
    "user.table.createdAt": "Ngày tạo",
    "user.table.updatedAt": "Ngày cập nhật",
    "user.sync.button": "Đồng bộ Firebase",
    "user.sync.success": "Đã đồng bộ {synced}/{total} người dùng từ Firebase",
    "user.sync.error": "Đồng bộ thất bại",
    "user.update.success": "Cập nhật thành công",
    "user.update.error": "Cập nhật thất bại",

    // Tag Table
    "tag.table.title": "Danh sách thẻ",
    "tag.table.name": "Tên thẻ",
    "tag.table.nameVi": "Tên tiếng Việt",
    "tag.table.nameEn": "Tên tiếng Anh",
    "tag.table.slug": "Slug",
    "tag.table.postCount": "Số bài viết",
    "tag.table.createdAt": "Ngày tạo",

    // Tag Form
    "tag.form.viewTitle": "Chi tiết thẻ",
    "tag.form.nameVi": "Tên tiếng Việt",
    "tag.form.nameVi.placeholder": "Nhập tên thẻ tiếng Việt",
    "tag.form.nameEn": "Tên tiếng Anh",
    "tag.form.nameEn.placeholder": "Nhập tên thẻ tiếng Anh",

    // Category Table
    "category.table.title": "Danh sách danh mục",
    "category.table.name": "Tên danh mục",
    "category.table.slug": "Slug",
    "category.table.parent": "Danh mục cha",
    "category.table.postCount": "Số bài viết",
    "category.table.createdAt": "Ngày tạo",

    // Category Form
    "category.form.nameVi": "Tên tiếng Việt",
    "category.form.nameVi.placeholder": "Nhập tên danh mục tiếng Việt",
    "category.form.nameEn": "Tên tiếng Anh",
    "category.form.nameEn.placeholder": "Nhập tên danh mục tiếng Anh",
    "category.form.description": "Mô tả",
    "category.form.description.placeholder": "Nhập mô tả danh mục",
    "category.form.parent": "Danh mục cha",
    "category.form.parent.placeholder": "Chọn danh mục cha",

    // Category Tree
    "category.tree.empty": "Chưa có danh mục nào. Nhấn nút Thêm mới để tạo danh mục đầu tiên.",
    "category.tree.reorderSuccess": "Đã cập nhật thứ tự danh mục",
    "category.tree.reorderError": "Cập nhật thứ tự thất bại",

    // Category Delete
    "category.delete.alertWithChildren":
        "Bạn có chắc muốn xóa {label}? Tất cả danh mục con cũng sẽ bị xóa. Hành động này không thể hoàn tác.",

    // Post Table
    "post.table.title": "Danh sách bài viết",
    "post.table.titleVi": "Tiêu đề (VI)",
    "post.table.titleEn": "Tiêu đề (EN)",
    "post.table.author": "Tác giả",
    "post.table.category": "Danh mục",
    "post.table.status": "Trạng thái",
    "post.table.publishedAt": "Ngày xuất bản",
    "post.table.createdAt": "Ngày tạo",
    "post.table.viewCount": "Lượt xem",
    "post.status.draft": "Nháp",
    "post.status.published": "Đã xuất bản",
    "post.status.archived": "Đã lưu trữ",

    // Post Form
    "post.form.viewTitle": "Chi tiết bài viết",
    "post.form.content": "Nội dung",
    "post.form.settings": "Cài đặt",
    "post.form.vietnamese": "Tiếng Việt",
    "post.form.english": "Tiếng Anh",
    "post.form.titleVi": "Tiêu đề tiếng Việt",
    "post.form.titleVi.placeholder": "Nhập tiêu đề tiếng Việt",
    "post.form.titleEn": "Tiêu đề tiếng Anh",
    "post.form.titleEn.placeholder": "Nhập tiêu đề tiếng Anh",
    "post.form.excerptVi": "Tóm tắt tiếng Việt",
    "post.form.excerptVi.placeholder": "Nhập tóm tắt tiếng Việt",
    "post.form.excerptEn": "Tóm tắt tiếng Anh",
    "post.form.excerptEn.placeholder": "Nhập tóm tắt tiếng Anh",
    "post.form.contentVi": "Nội dung tiếng Việt",
    "post.form.contentEn": "Nội dung tiếng Anh",
    "post.form.status": "Trạng thái",
    "post.form.category": "Danh mục",
    "post.form.category.placeholder": "Chọn danh mục",
    "post.form.thumbnail": "Thumbnail",
    "post.form.thumbnail.placeholder": "Nhập URL thumbnail",
    "post.form.tags": "Thẻ",
    "post.form.tags.placeholder": "Chọn thẻ",

    // Comment Table
    "comment.table.title": "Danh sách bình luận",
    "comment.table.content": "Nội dung",
    "comment.table.author": "Người bình luận",
    "comment.table.post": "Bài viết",
    "comment.table.parent": "Phản hồi của",
    "comment.table.status": "Trạng thái",
    "comment.table.createdAt": "Ngày tạo",
    "comment.status.pending": "Chờ duyệt",
    "comment.status.approved": "Đã duyệt",
    "comment.status.rejected": "Đã từ chối",

    // Auth
    "auth.welcome": "Chào mừng trở lại",
    "auth.subtitle": "Đăng nhập để tiếp tục quản lý blog",
    "auth.signIn": "Đăng nhập",
    "auth.signUp": "Đăng ký",
    "auth.continueWithGoogle": "Tiếp tục với Google",
    "auth.orEmail": "hoặc sử dụng email",
    "auth.email": "Email",
    "auth.password": "Mật khẩu",
    "auth.emailRequired": "Vui lòng nhập email",
    "auth.emailInvalid": "Email không hợp lệ",
    "auth.passwordRequired": "Vui lòng nhập mật khẩu",
    "auth.passwordMinLength": "Mật khẩu phải có ít nhất 6 ký tự",
    "auth.loginSuccess": "Đăng nhập thành công",
    "auth.signupSuccess": "Đăng ký thành công",

    // Upload
    "upload.dragText": "Kéo thả hoặc nhấp để tải lên",
    "upload.hint": "Hỗ trợ: JPG, PNG, GIF, WebP. Tối đa 2MB",
};
